...
while(temp2):
    ...
    try:
        ...
        if year > 2020:
            print(
                'Sorry, we can"t to look into the future. Please, try again\n')
        elif year <= 0:
            print('Incorrect input!Please, try again\n')
        else:
            temp2 = False
...
url = f'https://www.kinopoisk.ru/s/type/film/list/1/order/{option}/'
...
def scrap():
    ...
    while(i <= int(pages)):
        try:
            Scrap_Result = []
            Movie_info = []
            driver.get(f'{url}m_act[year]/{year}/page/{i}/')
...
message = f'Done!{len(title)} films sorted by {option}
'are waiting for u in Excel file "kinopoisk"'
print(message)
workbook.close()
driver.quit()
